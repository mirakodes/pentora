...
This section summarizes how to compare current benchmark results against the saved baseline for regression detection.

- Generate/update baseline (multi-sample recommended for stability):
  - `go test -bench=. -benchmem ./pkg/fingerprint -run ^$ -benchtime=1s -count=6 > pkg/fingerprint/testdata/baseline.txt`
- Compare current results to baseline with benchstat:
  - `go test -bench=. -benchmem ./pkg/fingerprint -run ^$ -benchtime=1s -count=6 | benchstat pkg/fingerprint/testdata/baseline.txt -`
- Notes:
  - Prefer `-count>=6` to enable confidence intervals; increase if variance is high.
  - Update the baseline only after intentional, validated changes.
  - Include large dataset benches in periodic runs to observe scaling.
